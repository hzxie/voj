...
#
# Deployment config is read from VOJ_* environment variables (resolved by the app
# at runtime; see voj.properties). The preflight and the launched jar honour the
# same ones, e.g.:
#   VOJ_DB_HOST / VOJ_DB_PORT / VOJ_DB_NAME / VOJ_DB_USERNAME / VOJ_DB_PASSWORD
#   VOJ_JMS_BROKER_URL / VOJ_JMS_BROKER_EMBEDDED (true/false) / VOJ_BASE_URL
#
set -euo pipefail
...
# The app reads VOJ_JMS_BROKER_EMBEDDED from the environment, which the exec'd jar
# inherits, so the broker mode flips without a rebuild and without a -D flag.
...
# Warn if the source voj.properties is newer than the packaged jar. Deployment
# values now come from VOJ_* env vars at runtime, but structural edits to
# voj.properties (new keys, changed placeholders) are still packaged into the jar,
# so they will not take effect until you rebuild — a confusing silent no-op.
...
if [ -f "$SRC_PROPS" ] && [ "$SRC_PROPS" -nt "$JAR" ]; then
  ...
  echo "      Structural changes to it are packaged at build time and will NOT take" >&2
  echo "      effect until you rebuild: scripts/build-jars.sh web (or mvn package -f web/pom.xml)." >&2
  echo "      (Plain value changes should use the VOJ_* environment variables instead.)" >&2
fi
...
preflight() {
  ...
  #
  # If VOJ_JMS_BROKER_EMBEDDED is left unset we auto-detect: use an external broker
  # when one is already listening, otherwise have the web app host an embedded one
  # (exported so the launched jar picks it up). Set VOJ_JMS_BROKER_EMBEDDED
  # explicitly to opt out of the auto-detection.
  if [ -n "${VOJ_JMS_BROKER_EMBEDDED+set}" ]; then
    if [ "$VOJ_JMS_BROKER_EMBEDDED" = "true" ]; then
      echo "==> ActiveMQ broker is embedded in the web app; skipping external broker check."
    elif ! tcp_open "$jms_host" "$jms_port"; then
      echo "ERROR: VOJ_JMS_BROKER_EMBEDDED=false but no broker is reachable at $jms_host:$jms_port." >&2
      echo "       Start the broker (e.g. 'activemq start') and retry," >&2
      echo "       or unset VOJ_JMS_BROKER_EMBEDDED to host one in the web app." >&2
      exit 1
    else
      echo "==> Checking ActiveMQ at $jms_host:$jms_port ... ok"
    fi
  elif tcp_open "$jms_host" "$jms_port"; then
    export VOJ_JMS_BROKER_EMBEDDED=false
    echo "==> Found an ActiveMQ broker at $jms_host:$jms_port; using it."
  else
    export VOJ_JMS_BROKER_EMBEDDED=true
...
echo "==> Starting voj.web (${VOJ_BASE_URL:-http://localhost:8080/voj}) ..."
