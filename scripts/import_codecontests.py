...
EXISTING_TAGS = {"greedy": 1, "dp": 2, "dynamic programming": 2, "dynamic-programming": 2}
...
SOURCE_NAME = {0: "competitive programming archives", 1: "CodeChef", 2: "Codeforces",
               3: "HackerEarth", 4: "Google Code Jam", 5: "AtCoder", 6: "Aizu"}

# Phrases that signal a non-exact-match (special judge / floating point / multiple answers).
SPECIAL = ("absolute or relative error", "relative or absolute error", "absolute error",
           "relative error", "considered correct if", "interactive problem", "print any",
           "output any", "any of them", "any correct", "any valid", "any suitable",
           "if there are several", "if there are multiple", "if there are many",
           "you can print", "you may print", "any solution", "multiple solutions")
BAD_TAGS = {"interactive", "*special", "schedules", "games", "probabilities",
...
def fetch(split, offset, length=100):
    ...
    req = urllib.request.Request(API + "?" + qs, headers={"User-Agent": "voj-importer/1.0"})
...
def gather_tests(row):
    ...
    def pairs(key):
        ...
        return [(clean(ins[i]), clean(outs[i])) for i in range(min(len(ins), len(outs)))]

    pub = pairs("public_tests")
    if not pub or not pub[0][0].strip() or not pub[0][1].strip():
        return None
    chosen = [pub[0]]
    pool = [p for p in (pub[1:] + pairs("private_tests") + pairs("generated_tests"))
    ...
    for split, total in plan:
        ...
        while off < total:
            ...
            for item in rows:
                ...
                mem_kb = max(65536, int(r.get("memory_limit_bytes") or 268435456) // 1024)
                cands.append({
                    "name": PREFIX.sub("", name)[:120], "rating": rating, "tags": tags,
                    "ms": ms, "mem_kb": mem_kb,
                    "source": SOURCE_NAME.get(r.get("source", 0), "competitive programming archives"),
                    "statement": parsed[0], "in_fmt": parsed[1], "out_fmt": parsed[2],
                    "tests": tests,
                })
            off += 100
            time.sleep(0.4)
        have = {t: sum(1 for c in cands if lo <= c["rating"] <= hi) for t, lo, hi in TIERS}
...
def pick(cands):
    ...
    for tier, lo, hi in TIERS:
        b = sorted([c for c in cands if lo <= c["rating"] <= hi], key=lambda c: (c["rating"], c["name"]))
        if len(b) > PER_TIER:
            idx = sorted(set(round(i * (len(b) - 1) / (PER_TIER - 1)) for i in range(PER_TIER)))
            b = [b[i] for i in idx]
...
def build_fragment(selected, tag_id):
    out = ["-- >>> CODECONTESTS BEGIN (generated by scripts/import_codecontests.py; do not edit)",
    ...
    cols = ("`problem_id`, `problem_is_public`, `problem_name`, `problem_time_limit`, `problem_memory_limit`, "
    ...
    for c, tier in selected:
        desc = html_safe(c["statement"]) + "\n\n---\n*Source: %s problem, via DeepMind CodeContests (CC BY 4.0).*" % c["source"]
        si, so = c["tests"][0]
        prows.append("(%d, 1, %s, %d, %d, %s, %s, %s, %s, %s, NULL)" % (
            pid, sql_str(c["name"]), c["ms"], c["mem_kb"], sql_str(desc),
            sql_str(html_safe(c["in_fmt"])), sql_str(html_safe(c["out_fmt"])),
            sql_str(html_safe(si)), sql_str(html_safe(so))))
    ...
    for tier, _, _ in TIERS:
        ...
        if ids:
            out.append("UPDATE `voj_problems` SET `problem_difficulty_id` = %d WHERE `problem_id` IN (%s);"
                       % (tier, ", ".join(ids)))

    out.append("INSERT INTO `voj_problem_checkpoints` (`problem_id`, `checkpoint_id`, "
    ...
    for p, c, t in meta:
        ...
        for i, (ti, to) in enumerate(c["tests"]):
            crows.append("(%d, %d, 0, %d, %s, %s)" % (p, i, scores[i], sql_str(ti), sql_str(to)))
    out.append(",\n".join(crows) + ";")

    out.append("INSERT INTO `voj_problem_category_relationships` (`problem_id`, `problem_category_id`) VALUES")
    catrows = []
    for p, c, t in meta:
    ...
    if trows:
        out.append("INSERT INTO `voj_problem_tag_relationships` (`problem_id`, `problem_tag_id`) VALUES")
        out.append(",\n".join(trows) + ";")
...
def splice_demo(fragment):
    with io.open(DEMO, "r", encoding="utf-8") as f:
        demo = f.read()
    demo = re.sub(r"-- >>> CODECONTESTS BEGIN.*?-- <<< CODECONTESTS END\n", "", demo, flags=re.S)
...
def rebuild_seed_tags(new_tags):
    with io.open(SEED, "r", encoding="utf-8") as f:
        seed = f.read()
    lines = ["(1, 'greedy', 'Greedy')", "(2, 'dynamic-programming', 'Dynamic Programming')"]
    for tid, t in new_tags:
        lines.append("(%d, %s, %s)" % (tid, sql_str(slugify(t)), sql_str(t.title())))
    insert = ("INSERT INTO `voj_problem_tags` (`problem_tag_id`, `problem_tag_slug`, `problem_tag_name`) VALUES\n"
              + ",\n".join(lines) + ";")
    seed = re.sub(r"INSERT INTO `voj_problem_tags`.*?;", insert, seed, count=1, flags=re.S)
...
def main():
    ...
    for p, c, t in meta:
        print("  %d  d%d  r%-4d  %2d cp  %s" % (p, t, c["rating"], len(c["tests"]), c["name"]))
